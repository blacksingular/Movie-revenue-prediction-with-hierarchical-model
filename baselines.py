...
class Regression:
    ...
    def pre_processing_gao(self):
        train_df = pd.DataFrame(pd.read_csv('./tables/omdb_train.csv'))
        test_df = pd.DataFrame(pd.read_csv('./tables/omdb_test.csv'))
        params = ["Language", "Country", "Actors", "Director", "Genre", "writer"]

        self.train_y = np.array(train_df["BoxOffice"])
        self.test_y = np.array(test_df["BoxOffice"])
        self.train_X = np.array(train_df.iloc[:, 1:9])
        self.test_X = np.array(test_df.iloc[:, 1:9])
        print(np.array(train_df["Year"]).shape)
